...
class RMC(NMEAGP):
    ...
    def parse(self):
        try:
            self._parse = re.search('\$GPRMC,(?P<timeutc>((?P<hour>\d{2})(?P<minute>\d{2})(?P<seconds>\d{2})\.(?P<microseconds>\d{3})))?,(?P<status>[AV]),(?P<latitude>\d{4}.\d{4})?,(?P<ns>[NS])?,(?P<longitude>\d{5}.\d{4})?,(?P<ew>[EW])?,(?P<speedOG>\d.\d{2})?,(?P<courseOG>\d{3}.\d{2}|0.00)?,(?P<date>(?P<day>\d{2})(?P<month>\d{2})(?P<year>\d{2}))?,(?P<magneticVar>\d{3}.\d{2})?,(?P<ewMG>[EW])?,[A-Z]?(\*\w{2})?',self._msg)
